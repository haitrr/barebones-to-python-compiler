...
def compiler():
    compilered_code_text_box.config(state=NORMAL)
    try:
        ast = Parser.parse(source_code_text_box.get(1.0,END), verbose=False)
    except Exception as e:
        compilered_code_text_box.delete(1.0, END)
        compilered_code_text_box.insert(INSERT,e )
        run_button.config(state=DISABLED)
    else:
        result = Compiler.compiler(ast)
        ...
        compilered_code_text_box.delete(1.0,END)
        compilered_code_text_box.insert(INSERT,result)
        save_result(result)
        run_button.config(state=NORMAL)
    finally:
        compilered_code_text_box.config(state=DISABLED)
...
compilered_code_text_box = ScrolledText(root,width = 50,height=30,state=DISABLED)
compilered_code_text_box.grid(row =2 ,column = 1)
